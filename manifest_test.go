...
func TestA2ASkillDef_InputModeParsing(t *testing.T) {
	cases := []struct {
		name    string
		yamlStr string
		want    string
	...
	for _, tt := range cases {
		...
		t.Run(tt.name, func(t *testing.T) {
			var def A2ASkillDef
			if err := yaml.Unmarshal([]byte(tt.yamlStr), &def); err != nil {
				t.Fatalf("yaml.Unmarshal: %v", err)
			}
			if def.InputMode != tt.want {
				t.Errorf("InputMode=%q want %q", def.InputMode, tt.want)
			}
		})
	}
}

func TestA2ASkillDef_ValidateInputMode(t *testing.T) {
	cases := []struct {
		mode  string
		valid bool
	}{
		{"", true}, // 空值合法（消费方默认 json）
		{"json", true},
		{"natural", true},
		{"unknown", false},
	}
	for _, c := range cases {
		...
		t.Run(c.mode, func(t *testing.T) {
			...
			gotValid := err == nil
			if gotValid != c.valid {
				t.Errorf("InputMode=%q valid=%v want %v (err=%v)", c.mode, gotValid, c.valid, err)
			}
		})
	}
}

// TestValidate_ExecutionModeRequired 缺 execution_mode 字段 → 启动期 fail-fast，
// 错误信息含 "execution_mode is required"（严格必填，破坏性 minor 变更）。
func TestValidate_ExecutionModeRequired(t *testing.T) {
	def := A2ASkillDef{
		Name:        "test",
		InputModes:  []string{"text"},
		OutputModes: []string{"text"},
	...
	err := def.Validate()
	if err == nil {
		t.Fatal("期望 ExecutionMode 缺失校验失败，实际通过")
	}
	if !strings.Contains(err.Error(), "execution_mode is required") {
		t.Errorf("错误信息应含 'execution_mode is required'，got: %v", err)
	}
}

// TestValidate_ExecutionModeInvalid 非法值（如 "streaming"）→ 启动期 fail-fast，
// 错误信息含 "execution_mode \"streaming\" invalid (allowed: sync|async)"。
func TestValidate_ExecutionModeInvalid(t *testing.T) {
	def := A2ASkillDef{
		Name:          "test",
		InputModes:    []string{"text"},
		OutputModes:   []string{"text"},
	...
	err := def.Validate()
	if err == nil {
		t.Fatal("期望 ExecutionMode 非法值校验失败，实际通过")
	}
	if !strings.Contains(err.Error(), `execution_mode "streaming" invalid`) {
		t.Errorf("错误信息应含 'execution_mode \"streaming\" invalid'，got: %v", err)
	}
	if !strings.Contains(err.Error(), "sync|async") {
		t.Errorf("错误信息应含合法枚举提示 'sync|async'，got: %v", err)
	}
}

// TestValidate_ExecutionModeSync sync 是合法值（短查询类 skill 同步等终态返回）。
func TestValidate_ExecutionModeSync(t *testing.T) {
	def := A2ASkillDef{
		Name:          "test",
		InputModes:    []string{"text"},
		OutputModes:   []string{"text"},
	...
	if err := def.Validate(); err != nil {
		t.Errorf("ExecutionMode=sync 应通过校验，got: %v", err)
	}
}

// TestValidate_ExecutionModeAsync async 是合法值（长任务类 skill fire-and-forget）。
func TestValidate_ExecutionModeAsync(t *testing.T) {
	def := A2ASkillDef{
		Name:          "test",
		InputModes:    []string{"text"},
		OutputModes:   []string{"text"},
	...
	if err := def.Validate(); err != nil {
		t.Errorf("ExecutionMode=async 应通过校验，got: %v", err)
	}
}

func TestA2ASkillDef_InputModeRoundTripJSON(t *testing.T) {
	def := A2ASkillDef{
		Name:            "test",
		InputModes:      []string{"text"},
		OutputModes:     []string{"text"},
	...
	if !strings.Contains(string(data), `"inputMode":"natural"`) {
		t.Errorf("expected inputMode in JSON, got: %s", data)
	}
	var got A2ASkillDef
	if err := json.Unmarshal(data, &got); err != nil {
		t.Fatalf("json.Unmarshal: %v", err)
	}
	if got.InputMode != "natural" {
		t.Errorf("round-trip InputMode: got %q want natural", got.InputMode)
	}
	// AcceptsBindings 字段不破现有 InputMode round-trip（minor 兼容扩展）
	...
	// omitempty：InputMode 为空时不出现在 JSON 中
	// 用精确字符串 `"inputMode":` 区分；inputModes 后跟 `[`，inputMode 后跟字符串值（"）
	emptyDef := A2ASkillDef{Name: "x", InputModes: []string{"text"}, OutputModes: []string{"text"}, ExecutionMode: "sync"}
	emptyData, _ := json.Marshal(emptyDef)
	if strings.Contains(string(emptyData), `"inputMode":"`) {
		t.Errorf("expected inputMode omitted, got: %s", emptyData)
	}
}

// TestA2ASkillDef_AcceptsBindings_BackwardCompat 验证不声明 accepts_bindings 的旧 manifest 仍能正常解析。
// minor 兼容扩展硬约束：旧 squad manifest 不破。
func TestA2ASkillDef_AcceptsBindings_BackwardCompat(t *testing.T) {
	yamlStr := `
name: legacy_skill
...
`
	...
	if def.AcceptsBindings != nil {
		t.Errorf("expected AcceptsBindings nil for legacy YAML manifest, got %v", def.AcceptsBindings)
	}

	jsonStr := `{"name":"legacy_skill","description":"旧版 manifest","inputModes":["text"],"outputModes":["text"]}`
	var def2 A2ASkillDef
	if err := json.Unmarshal([]byte(jsonStr), &def2); err != nil {
		t.Fatalf("json.Unmarshal: %v", err)
	}
	if def2.AcceptsBindings != nil {
		t.Errorf("expected AcceptsBindings nil for legacy JSON, got %v", def2.AcceptsBindings)
	}
}

// TestA2ASkillDef_AcceptsBindings_RoundTripYAML 验证 accepts_bindings YAML 解析与 round-trip。
func TestA2ASkillDef_AcceptsBindings_RoundTripYAML(t *testing.T) {
	yamlStr := `
name: develop
description: 代码开发主 Skill
input_modes: [text]
output_modes: [text]
accepts_bindings:
  - landing_page_repo
...
`
	...
	if strings.Join(def.AcceptsBindings, "|") != "landing_page_repo|scripts_repo" {
		t.Errorf("AcceptsBindings YAML parse: got %v want [landing_page_repo, scripts_repo]", def.AcceptsBindings)
	}
	...
	if !strings.Contains(string(data), "accepts_bindings:") {
		t.Errorf("expected accepts_bindings in YAML output, got: %s", data)
	}
	var got A2ASkillDef
	if err := yaml.Unmarshal(data, &got); err != nil {
		t.Fatalf("yaml.Unmarshal round-trip: %v", err)
	}
	if strings.Join(got.AcceptsBindings, "|") != "landing_page_repo|scripts_repo" {
		t.Errorf("round-trip AcceptsBindings: got %v", got.AcceptsBindings)
	}

	// omitempty：未声明时不出现在 YAML 中
	emptyDef := A2ASkillDef{Name: "x", InputModes: []string{"text"}, OutputModes: []string{"text"}, ExecutionMode: "sync"}
	emptyData, _ := yaml.Marshal(emptyDef)
	if strings.Contains(string(emptyData), "accepts_bindings:") {
		t.Errorf("expected accepts_bindings omitted for empty def, got: %s", emptyData)
	}
}

// TestA2ASkillDef_AcceptsBindings_RoundTripJSON 验证 acceptsBindings JSON 解析与 round-trip（camelCase tag）。
func TestA2ASkillDef_AcceptsBindings_RoundTripJSON(t *testing.T) {
	def := A2ASkillDef{
		Name:            "develop",
		Description:     "代码开发主 Skill",
		InputModes:      []string{"text"},
		OutputModes:     []string{"text"},
		AcceptsBindings: []string{"landing_page_repo", "scripts_repo"},
		ExecutionMode:   "sync",
	}
	data, err := json.Marshal(def)
	if err != nil {
	...
	if !strings.Contains(string(data), `"acceptsBindings":["landing_page_repo","scripts_repo"]`) {
		t.Errorf("expected acceptsBindings camelCase in JSON, got: %s", data)
	}
	var got A2ASkillDef
	if err := json.Unmarshal(data, &got); err != nil {
		t.Fatalf("json.Unmarshal: %v", err)
	...
	// omitempty：未声明时不出现在 JSON 中
	emptyDef := A2ASkillDef{Name: "x", InputModes: []string{"text"}, OutputModes: []string{"text"}, ExecutionMode: "sync"}
	emptyData, _ := json.Marshal(emptyDef)
	if strings.Contains(string(emptyData), `"acceptsBindings":`) {
		t.Errorf("expected acceptsBindings omitted, got: %s", emptyData)
	}
}
