...
// A2AConfig manifest 中 a2a 子段，声明 Agent-to-Agent 能力。
//
// Deprecated: manifest 声明层已不再承载 a2a，AppSpec 不再有 A2A 字段。
// 类型本体暂留供 a2a 协议栈过渡，待 a2a 协议栈退役后完整删除。
type A2AConfig struct {
	Enabled         bool                   `yaml:"enabled" json:"enabled"`
	ProtocolVersion string                 `yaml:"protocol_version,omitempty" json:"protocolVersion,omitempty"`
	Skills          map[string]A2ASkillDef `yaml:"skills,omitempty" json:"skills,omitempty"`
}

// A2ASkillDef 单个 A2A skill 的能力描述。
//
// Deprecated: 同 A2AConfig——manifest 声明层不再承载 a2a，类型暂留供 a2a 协议栈过渡，
// 待 a2a 协议栈退役后完整删除。
type A2ASkillDef struct {
	Name        string   `yaml:"name" json:"name"`
	Description string   `yaml:"description" json:"description"`
	InputModes  []string `yaml:"input_modes" json:"inputModes"`
	OutputModes []string `yaml:"output_modes" json:"outputModes"`
	Examples    []string `yaml:"examples,omitempty" json:"examples,omitempty"`
	// InputMode 入参形态：json（默认）| natural（自然语言，需 LLM 解析）
	// 空值由消费方判定为 json，向后兼容早期 manifest（无该字段）
	InputMode string `yaml:"input_mode,omitempty" json:"inputMode,omitempty"`
	// AcceptsBindings 列出本 skill 可消费的集成绑定语义键（binding_kind，
	// 按集成绑定约定的注册表）。
	// 空值/未声明 → 不消费 binding 注入，沿早期透传 args.query 行为。
	// minor 兼容扩展（v0.9.3）；omitempty 保证旧 manifest 解析不破。
	AcceptsBindings []string `yaml:"accepts_bindings,omitempty" json:"acceptsBindings,omitempty"`

	// ExecutionMode 必填：sync / async
	//   - sync:  squad 在 tasks/send 响应中返终态 task（含内部短轮询 / SSE 等终态）；
	//            keystone Agent 拿到结果后立即 LLM next loop
	//   - async: squad 收到 tasks/send 后立即返 working 状态，后续通过 push notification 推进；
	//            keystone Agent 视作 fire-and-forget
	// 现严格必填 + Validate 枚举校验（破坏性 minor 变更，旧 manifest 启动期 fail-fast）。
	ExecutionMode string `yaml:"execution_mode" json:"executionMode"`
}

// Validate 校验 A2ASkillDef 的必填字段
func (d A2ASkillDef) Validate() error {
	if d.Name == "" {
		return fmt.Errorf("name is required")
	}
	if len(d.InputModes) == 0 {
		return fmt.Errorf("input_modes is required")
	}
	if len(d.OutputModes) == 0 {
		return fmt.Errorf("output_modes is required")
	}
	switch d.InputMode {
	case "", "json", "natural":
		// ok
	default:
		return fmt.Errorf("input_mode %q invalid (allowed: json|natural)", d.InputMode)
	}
	switch d.ExecutionMode {
	case "sync", "async":
		// ok
	case "":
		return fmt.Errorf("execution_mode is required (allowed: sync|async)")
	default:
		return fmt.Errorf("execution_mode %q invalid (allowed: sync|async)", d.ExecutionMode)
	}
	return nil
}
