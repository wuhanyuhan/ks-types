...
func TestMetaResponse_Capabilities_OmitEmpty(t *testing.T) {
	t.Parallel()
	m := MetaResponse{Name: "x", Version: "1"}
	b, err := json.Marshal(m)
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
	if strings.Contains(string(b), `"capabilities"`) {
		t.Errorf("capabilities should be omitted when nil, got: %s", string(b))
	}
}

func TestMetaResponse_Capabilities_UI_Enabled(t *testing.T) {
	t.Parallel()
	m := MetaResponse{
		Name: "x", Version: "1",
		Capabilities: &MetaCapabilities{
			UI: &CapabilitiesUI{Enabled: true, RequestedSandbox: []string{"allow-downloads"}},
		},
	}
	b, err := json.Marshal(m)
	if err != nil {
	...
	expectFragment := `"capabilities":{"ui":{"enabled":true,"requested_sandbox":["allow-downloads"]}}`
	if !strings.Contains(string(b), expectFragment) {
		t.Errorf("missing fragment %s in %s", expectFragment, string(b))
	}
}

func TestToolInfo_Meta_UI_Binding(t *testing.T) {
	t.Parallel()
	tool := ToolInfo{
		Name: "review_draft",
		Meta: &ToolInfoMeta{
			UI: &ToolUIBinding{Widget: "ks://widgets/diff-review@v1"},
		},
	}
	...
	if !strings.Contains(string(b), `"_meta":{"ui":{"widget":"ks://widgets/diff-review@v1"}}`) {
		t.Errorf("missing _meta.ui.widget in: %s", string(b))
	}
}

func TestToolInfo_NoMeta_OmitEmpty(t *testing.T) {
	t.Parallel()
	tool := ToolInfo{Name: "list_drafts"}
	b, err := json.Marshal(tool)
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
	if strings.Contains(string(b), `"_meta"`) {
		t.Errorf("_meta should be omitted when nil, got: %s", string(b))
	}
}
