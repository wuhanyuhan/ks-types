...
// MetaNavDecl 是 MCP 自声明的左侧菜单项（v0.5.0 新增）。
...
type MetaNavDecl struct {
	...
	OpenMode      string   `json:"open_mode"`                // dialog / fullpage / tab（tab：浏览器新标签打开，适合长任务/沉浸式 SPA）
...
type MetaResponse struct {
	...
	//   - ConfigMode == "iframe"   → 必须同时填 ConfigUI.URL（启动期校验）
	//   - ConfigMode == "schema"   → ConfigUI 应为 nil（配置由 SchemaForm 渲染）
	...
	// ConfigStatus unconfigured / via_frontend / via_cli / mixed（v0.5.0 新增）。
	ConfigStatus string `json:"config_status,omitempty"`
	// Capabilities 服务声明的能力（v0.6.0 widgets-protocol-v1 新增）。
	// 平台侧 mount 时按 capabilities.ui.enabled 决定是否
	// 处理 tools[]._meta.ui binding。
	Capabilities *MetaCapabilities `json:"capabilities,omitempty"`
}
...
type ToolInfo struct {
	...
	// Meta tool 级元数据（v0.6.0 widgets-protocol-v1 新增）。
	// 当前承载 _meta.ui binding；未来可扩展其他 tool-level 字段。
	Meta *ToolInfoMeta `json:"_meta,omitempty"`
}

// MetaCapabilities 是 /meta.capabilities 字段（v0.6.0 widgets-protocol-v1 新增）。
type MetaCapabilities struct {
	UI *CapabilitiesUI `json:"ui,omitempty"`
}

// ToolInfoMeta 是 /meta.tools[]._meta 字段（v0.6.0 widgets-protocol-v1 新增）。
type ToolInfoMeta struct {
	UI *ToolUIBinding `json:"ui,omitempty"`
}
