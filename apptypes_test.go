...
func TestProtectionLevelValid(t *testing.T) {
	valid := []ProtectionLevel{
		"", ProtectionNone, ProtectionProtected, ProtectionSystem,
	}
	for _, p := range valid {
		if !p.Valid() {
...
func TestProtectionLevel_NoPreinstalled(t *testing.T) {
	if ProtectionLevel("preinstalled").Valid() {
		t.Error("preinstalled 死值应已清，不应再合法")
	}
...
func TestAppType_FourTypes(t *testing.T) {
	for _, ty := range []AppType{AppTypeApp, AppTypeSquad, AppTypeAgent, AppTypeSkill} {
		if !ty.Valid() {
			t.Errorf("%q 应合法", ty)
		}
	}
	// 旧四类型（service/assistant/extension）clean-break 废弃，不再合法。
	for _, ty := range []string{"service", "assistant", "extension"} {
		if AppType(ty).Valid() {
			t.Errorf("%q 应已废弃（不再合法）", ty)
		}
	}
}
